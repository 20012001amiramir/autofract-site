...
Display headings run at a 44px (hero) / 34px (chapter) floor. `hyphens: auto` is set and `lang` is
correct per locale, but Chromium only ships a hyphenation dictionary for some of the six languages,
so a compound wider than its column — `Kleingedruckte`, `Ferramentas`, `Construimos` — carries a soft
hyphen (U+00AD) at a real syllable boundary. `overflow-wrap: break-word` stays only as the guard
against a genuinely unbreakable token; nothing in the copy should ever reach it, because reaching it
means a word chopped bare with no hyphen at all.

Two tests hold that line. `tests/unit/typography.spec.ts` measures every headline string against real
Fraunces Black advance widths (`tests/fixtures/display-widths.json`, regenerated by
`scripts/measure-type.mjs`) and fails on any word wider than the column its role renders in — the
per-role budgets there are the contract with the `fontSize` scale in `tailwind.config.ts`. It also
fails on a soft hyphen leaking into a `<title>` or a description. In `tests/e2e/locales.spec.ts`,
`no display heading chops a word without a hyphen` renders every narrow page at 320/360/414 and
compares line-break positions with and without the guard, so a bare chop is caught in a real browser
